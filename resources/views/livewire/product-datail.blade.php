        ...
        <div class="flex flex-col md:flex-row items-center gap-x-8">
            ...
            <div class="space-y-4 mt-4 md:mt-0">
                ...
                <button data-product-id="{{ $product->id }}"
                    class="bg-purple-600 w-full py-2 rounded-lg text-white font-semibold openModal">Payer
                    maintenant</button>
            </div>
        </div>
    </section>
    ...
    <section class="mx-auto max-w-5xl relative pt-28 px-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 pt-12">
            @foreach ($products as $product)
                <div
                    class="relative rounded-xl overflow-hidden shadow-lg border border-gray-200 hover:shadow-xl transition duration-300">
                    @php
                        foreach ($imageExtensions as $extension) {
                            $path = public_path('img/' . $product->id . '.' . $extension);
                            if (file_exists($path)) {
                    ...
                    @endphp
                    @if ($imagePath)
                        <img src="{{ $imagePath }}" class="h-[300px] w-full object-cover object-center"
                            alt="{{ $product->title }}">
                    @else
                        <img src="{{ asset('img/default.jpg') }}" alt="Image par défaut">
                    @endif
            ...
            <form id="paymentForm" method="post" action="{{ route('payment') }}">
                ...
                <div class="flex justify-end space-x-4">
                    <button type="button" id="closeModal"
                        class="bg-gray-600 text-white px-4 py-2 rounded-lg">Annuler</button>
                    <button type="submit" class="bg-purple-600 text-white px-4 py-2 rounded-lg">Confirmer et
                        Payer</button>
                </div>
            </form>
        </div>
